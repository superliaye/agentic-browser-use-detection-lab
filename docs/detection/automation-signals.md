...
The detector includes two generic automation probes and two informational probes:
...
`cdp-runtime-serialization-observer` is a best-effort observation of Runtime console serialization, not CDP generally. Manually opened DevTools or any other CDP client that enables the Runtime domain can trigger it, while a client that never enables Runtime can remain undetected. The technique is undocumented and Chromium-version-sensitive. Once its one-time side effect is observed, the signal remains detected for the detector session; this does not prove that an observer is still attached.
...
- [Chrome DevTools MCP 1.7.0 browser launch](https://github.com/ChromeDevTools/chrome-devtools-mcp/blob/chrome-devtools-mcp-v1.7.0/src/browser.ts)
- [Puppeteer 25.5.0 mouse input](https://github.com/puppeteer/puppeteer/blob/puppeteer-v25.5.0/packages/puppeteer-core/src/cdp/Input.ts)
...
- [Castle — obsolete `Error.stack` CDP serialization technique](https://blog.castle.io/why-a-classic-cdp-bot-detection-signal-suddenly-stopped-working-and-nobody-noticed/)
